    ...
    <BODY>
      ...
      <li>
        ...
        <FORM  method="post" class="bigTable">
             ...
             <tbody>
               <c:forEach var="object" items="${objects}">
                <TR>
                    ...
                    <c:choose>
                        <c:when test="${object.role eq 0}">
                                <td>admin</td>
                        </c:when>
                        <c:when test="${object.role eq 1}">
                                <td>cashier</td>
                        </c:when>
                        <c:when test="${object.role eq 2}">
                                <td>user</td>
                        </c:when>
                        <c:otherwise>
                                <td>${object.role}</td>
                    ...
                    <c:if test="${role eq 0}">
                    <TD><A href="Uedit.html?id=${object.id}&page=${page}">&#128393;</A></TD>

                    <td><a href="Udelete.html?id=${object.id}&page=${page}"  >&#128937;</a></td></c:if></tr>
               </c:forEach>
            </TABLE>
            </FORM>
            <div class="col-3 col-sm-2 col-md-2 col-lg-1">
                <select onchange="window.location.href=this.options[this.selectedIndex].value" class="form-control" id="Select"
                    ...
                    <c:choose>
                            ...
                            <c:forEach var="id" begin="0" end="${allId-1}" step="5">
                                ...
                                <c:choose>
                                    <c:when test="${i eq page}">
                                        <option selected value='users.html?page=${i}'>${i}</option>
                                    </c:when>
                                    <c:otherwise>
                                        <option value='users.html?page=${i}'>${i}</option>
                                    </c:otherwise>
                                </c:choose>
                        ...
                        <c:otherwise>
                            <option value='users.html?page=1'>1</option>
                        </c:otherwise>
                    </c:choose>
         ...
         <br>
         <c:if test="${role eq 0}"> 
        <div class="col-3 col-sm-2 col-md-2 col-lg-2"><A href="Uedit.html?page=${page}">Add</A></div>
         </c:if>
    </BODY>
</HTML>